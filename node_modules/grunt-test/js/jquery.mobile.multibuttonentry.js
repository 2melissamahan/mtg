( function ( $, undefined ) {
 $.widget( "mobile.multibuttonentry", $.mobile.widget, {
		options : { // 위젯의 개별적인 속성, key:value 의 쌍으로 구성 
			label: "Test : ",
			initSelector : ":jqmData(role='multibuttonentry')"  // 적용 element 검색을 위한 셀렉터 
		},
		_minInputWidth : 50,
		_maxInputWidthRate : 0.6,
		_focusState : true,

		_create : function() { // element 와 option 을 this 로 접근 
			var self = this,
				option = this.options,
		...
		_addTextBtn: function ( msg, index ) {
			...
			if( checkExist ) {
				alert( "Already added [" + msg + "], input another text" );
			} else {
				if( index >= 0 ){
		...
		_resizeInputBox: function () {
			...
			$inputBox.width( parseInt( inputWidth - inputPaddingMargin ) );
		},
